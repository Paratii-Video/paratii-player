...
*/
const setupContractFilters = function (newDocument) {
  const contractInstance = tokenContracts[`ct_${newDocument._id}`] = TokenContract.at(newDocument.address);

  if (!contractInstance) { return; }

  let blockToCheckBack = (newDocument.checkpointBlock || 0) - ethereumConfig.rollBackBy;

    // TODO change to 0, when new geth is out!!!!!
  if (blockToCheckBack < 400000) { blockToCheckBack = 400000; }

  if (!contractInstance.tokenEvents) { contractInstance.tokenEvents = []; }

  const events = contractInstance.tokenEvents;
  ...
  const filter = contractInstance.allEvents({ fromBlock: blockToCheckBack, toBlock: 'latest' });
  events.push(filter);
  ...
  filter.watch(function (error, log) {
    if (!error) {
      ...
      if (log.event === 'Transfer' &&
        ...
        const txExists = addTransaction(log, log.args.from, log.args.to, log.args.value.toString(10));
  ...
  collectionObservers[collectionObservers.length] = Tokens.find({}).observe({
    ...
    added(newDocument) {
      ...
      web3.eth.getCode(newDocument.address, function (e, code) {
        ...
        */
    removed(document) {
      const contractInstance = tokenContracts[`ct_${document._id}`];
