...
addTransaction = function (log, from, to, value) {
  const txId = Helpers.makeId('tx', log.transactionHash);
  ...
  const block = web3.eth.getBlock(log.blockNumber, false, function (err, block) {
    if (!err) {
      web3.eth.getTransaction(log.transactionHash, function (err, transaction) {
        if (!err && transaction) {
          web3.eth.getTransactionReceipt(log.transactionHash, function (err, receipt) {
            ...
            if (log.args.operation) { tx.operation = log.args.operation; }
...
var updateTransaction = function (newDocument, transaction, receipt) {
  const id = newDocument._id || Helpers.makeId('tx', transaction.transactionHash || newDocument.transactionHash);

    // if transaction has no transactionId, stop
  if (!id) { return; }

  const oldTx = Transactions.findOne({ _id: id });
  ...
  if (transaction) {
    ...
    if (transaction.transactionHash) { newDocument.transactionHash = transaction.transactionHash; }

    newDocument.data = transaction.input || transaction.data || null;
    if (_.isString(newDocument.data) && newDocument.data === '0x') { newDocument.data = null; }
      ...
      web3.eth.getCode(receipt.contractAddress, function (e, code) {
        if (!e && code.length > 2) {
          ...
          if (oldTx && oldTx.jsonInterface) {
            ...
            const functionNames = _.pluck(oldTx.jsonInterface, 'name');
            const isToken = _.contains(functionNames, 'transfer') && _.contains(functionNames, 'Transfer') && _.contains(functionNames, 'balanceOf');
            ...
            if (isToken) {
              ...
              const tokenInstance = TokenContract.at(receipt.contractAddress);
  ...
  if (newDocument.outOfGas) {
    const warningText = TAPi18n.__('wallet.transactions.error.outOfGas', { from: Helpers.getAccountNameByAddress(newDocument.from), to: Helpers.getAccountNameByAddress(newDocument.to) });
...
observeTransactions = function () {
    ...
    */
  const checkTransactionConfirmations = function (tx) {
    let confCount = 0;
    ...
    if (!tx.confirmed && tx.transactionHash) {
      ...
      filter.watch(function (e, blockHash) {
        if (!e) {
          const confirmations = (tx.blockNumber && EthBlocks.latest.number) ? (EthBlocks.latest.number + 1) - tx.blockNumber : 0;
          confCount++;
            ...
            web3.eth.getTransaction(tx.transactionHash, function (e, transaction) {
              web3.eth.getTransactionReceipt(tx.transactionHash, function (e, receipt) {
                if (e || !receipt || !transaction) return;
                                ...
                                // update with receipt
                if (transaction.blockNumber !== tx.blockNumber) { updateTransaction(tx, transaction, receipt); }
              ...
              web3.eth.getTransactionReceipt(tx.transactionHash, function (e, receipt) {
                if (!e) {
                  ...
                  if (!transaction || !transaction.blockNumber) {
                    const warningText = TAPi18n.__('wallet.transactions.error.outOfGas', { from: Helpers.getAccountNameByAddress(tx.from), to: Helpers.getAccountNameByAddress(tx.to) });
  ...
  collectionObservers[collectionObservers.length] = Transactions.find({}).observe({
        ...
        */
    added(newDocument) {
      const confirmations = EthBlocks.latest.number - newDocument.blockNumber;
      ...
      if (newDocument.timestamp &&
        ...
        let url = `https://min-api.cryptocompare.com/data/pricehistorical?fsym=ETH&tsyms=BTC,USD,EUR&ts=${newDocument.timestamp}`;

        if (typeof mist !== 'undefined') { url += `&extraParams=Mist-${mist.version}`; }
